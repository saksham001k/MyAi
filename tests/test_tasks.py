...
class TaskTests(unittest.TestCase):
    def setUp(self):
        ...
        self.source.mkdir()
        (self.source / 'math.py').write_text('answer = 1\n')
        (self.root / 'app').mkdir()
    def tearDown(self):
        self.temp.cleanup()
    ...
    def test_project_write_verify_review_apply_and_undo(self):
        ...
        r = m.start({'goal': 'update', 'project_path': str(self.source), 'allow_commands': True,
                     'test_command': f'{sys.executable} -c "import math; assert math.answer == 2"'})
        ...
        self.assertEqual((self.source / 'math.py').read_text(), 'answer = 2\n')
    ...
    def test_failed_verification_never_reports_success(self):
        m = self.manager([action('write_file', path='math.py', content='answer = 2\n'), '{"final":"All tests passed!"}'])
