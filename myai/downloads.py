...
def _require_https(url, allow_loopback=False):
    if allow_loopback and urlparse(url).scheme == "http" and urlparse(url).hostname == "127.0.0.1":
        return
...
def download_file(url, destination, digest, progress=None, cancel=None, allow_loopback=False):
    ...
    if destination.is_symlink() or destination.with_name(destination.name + ".part").is_symlink():
        raise ValueError("Download paths cannot be symbolic links")
    ...
    try:
        with http.open(request, timeout=60) as response:
            _require_https(response.geturl(), allow_loopback=allow_loopback)
    ...
    if cancel is not None and cancel.is_set():
        raise DownloadCancelled("Download cancelled")
    if destination.exists():
        raise ValueError("Destination appeared during download; partial file retained.")
...
class DownloadManager:
    ...
    def start(self, item, kind="model"):
        ...
        with self.lock:
            if self.state["status"] in {"starting", "downloading", "verified"} or (self.thread is not None and self.thread.is_alive()):
        ...
        def run():
            try:
                ...
                download_file(
                    item["url"], target, digest, progress=lambda fields: self._set(**fields), cancel=self.cancel
                )
